...
// Update NodeStatus component to only show Codex node
const NodeStatus = ({ 
  isCodexNodeActive, 
  codexNodeUrl
}: { 
  ...
  const handleCopyCommand = () => {
    navigator.clipboard.writeText('npx codexstorage');
    setCopied(true);
    ...
    <div className="h-full flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader className="flex gap-2">
          <img src="/codex.svg" alt="Codex Logo" className="w-20 h-20 text-green-500" />
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <p className="text-lg font-mono font-medium">NODE_NOT_DETECTED</p>
            <p className="text-sm text-muted-foreground">
              Configure your node in the settings or run the following command in your terminal
            </p>
          </div>
    ...
    <div className={cn("min-h-screen bg-background font-sans antialiased", geistSans.variable, geistMono.variable)}>
      ...
      <header className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 pt-4">
        <div className="container mx-auto px-4 h-16 flex items-center">
          ...
          <div className="w-[200px] flex items-center gap-3 group">
          ...
          {/* Room ID Section - Center - Fixed width */}
          <div className="hidden md:flex flex-1 justify-center">
            <div className="inline-flex items-center gap-2 border border-border rounded-md px-4 py-2 bg-card shadow-sm w-[350px] relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col p-4 md:p-8 relative z-0">
        <div className="w-full max-w-5xl mx-auto">
          {/* Room ID Section - Only shown on mobile */}
          <div className="md:hidden flex items-center justify-center mb-8 pb-4">
            <div className="inline-flex items-center gap-2 border border-border rounded-md px-4 py-2 bg-card shadow-sm w-full relative overflow-hidden">
              <span className="text-sm font-medium text-secondary-foreground whitespace-nowrap font-mono">Room ID:</span>
          ...
          {isCodexNodeActive ? (
            // Show the main file transfer interface when Codex node is running
            <>
              {/* Upload Area */}
              <div 
                {...getRootProps()} 
            ...
            // Show the node status component when Codex node isn't running
            <NodeStatus 
              isCodexNodeActive={isCodexNodeActive}
              codexNodeUrl={codexNodeUrl}
            />
          )}
        </div>
      </main>
